...
impl RpcClient {
    ...
    pub async fn get_block(&self, num: u64, full_txs: bool) -> Result<TempoBlock> {
        let resp: RpcResponse<TempoBlock> = self
            .call(
    ...
    pub async fn get_blocks_batch(&self, range: RangeInclusive<u64>) -> Result<Vec<TempoBlock>> {
        ...
        let responses: Vec<RpcResponse<TempoBlock>> = self
            .client
            .post(&self.url)
            .json(&batch)
            .send()
    ...
    pub async fn get_block_receipts(&self, block_num: u64) -> Result<Vec<TempoReceipt>> {
        let resp: RpcResponse<Vec<TempoReceipt>> = self
            .call(
    ...
    pub async fn get_receipts_batch(&self, range: RangeInclusive<u64>) -> Result<Vec<Vec<TempoReceipt>>> {
        let batch: Vec<_> = range
            .clone()
            .enumerate()
            .map(|(i, n)| RpcRequest {
        ...
        let responses: Vec<RpcResponse<Vec<TempoReceipt>>> = self
            .client
            .post(&self.url)
            .json(&batch)
            .send()
    ...
    pub async fn get_logs(&self, from: u64, to: u64) -> Result<Vec<TempoLog>> {
        ...
        while start <= to {
            ...
            let resp: RpcResponse<Vec<TempoLog>> = self
                .call(
                    "eth_getLogs",
                    serde_json::json!([{
